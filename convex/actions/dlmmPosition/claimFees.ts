...
import { fastTransactionConfirm, getCuInstructions, toAddress, toVersioned } from "../../utils/solana";
...
async function buildAndSimulateClaimFeeTx({
  ...
  const claimFeeTx = await dlmmPoolConn.claimSwapFee({
  ...
  const tx = claimFeeTx[0]; //should loop all txs
  //remove Meteora's compute limit and use our own .
  const filteredIxs = tx.instructions.filter((ix) => {
